...
class M_recurring_invoices extends CI_Model{
    ...
    function get_sales($sales_id = FALSE, $from_date = null, $to_date=null)
    {
        ...
        if($sales_id == FALSE)
        {
            $query = $this->db->get_where('pos_recurring_invoices',array('company_id'=> $_SESSION['company_id']));
            return $query->result_array();
        }
        
       $query = $this->db->get_where('pos_recurring_invoices',array('sale_id'=>$sales_id,'company_id'=> $_SESSION['company_id']));
       return $query->result_array();
    }
    ...
    function get_selected_sales($from_date = null, $to_date=null)
    {
        ...
        $query = $this->db->get_where('pos_recurring_invoices as s',array('s.company_id'=> $_SESSION['company_id']));
        return $query->result_array();
    ...
    function get_creditSales($customer_id)
    {
       ...
       $query = $this->db->get_where('pos_recurring_invoices',array('account'=>'credit','register_mode'=>'sale',
    ...
    function updatePaidAmount($invoice_no,$data)
    {
       ...
       $this->db->update('pos_recurring_invoices',$data,array('invoice_no'=>$invoice_no,'company_id'=> $_SESSION['company_id']));
...
//       $this->db->join('pos_recurring_invoices_items as B','A.sale_id = B.sale_id');
//       $query = $this->db->get_where('pos_recurring_invoices as A',array('A.invoice_no'=>$new_invoice_no,'A.company_id'=> $_SESSION['company_id']));
//       return $query->result_array();
//       
//    }
    ...
    {   
        ...
        $query = $this->db->get('pos_recurring_invoices');
        return $query->result_array();
    ...
    //    $this->db->join('pos_recurring_invoices_items as B','A.sale_id = B.sale_id');
       ...
       $this->db->where(array('invoice_no'=>$invoice_no,'company_id'=>$_SESSION['company_id']));
       $query = $this->db->get('pos_recurring_invoices_items');
       return $query->result_array();
    ...
    {   
        ...
        $query = $this->db->get('pos_recurring_invoices',1);
    ...
    public function get_totalCostBysaleID($invoice_no)
    {
       ...
       $query = $this->db->get_where('pos_recurring_invoices_items',array('invoice_no'=>$invoice_no));
       $rows = $query->row();
       if($rows)
       {
    ...
    {
        $this->db->delete('pos_recurring_invoices',array('invoice_no'=>$invoice_no,'company_id'=> $_SESSION['company_id']));
        
        $this->db->delete('pos_recurring_invoices_items',array('invoice_no'=>$invoice_no,'company_id'=> $_SESSION['company_id']));
    ...
    public function get_totalSalesByCategory()
    {
        ...
        $this->db->join('pos_recurring_invoices_items as rt','rt.item_id = it.item_id','left');
        $this->db->join('pos_recurring_invoices as r','r.sale_id = rt.sale_id','left');
