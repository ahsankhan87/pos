...
class M_customers extends CI_Model
{
    ...
    {
        $this->db->select('id,first_name,store_name,address,mobile_no,city,op_balance_dr,op_balance_cr');
    ...
    function getCustomerDropDown()
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $data[$row['id']] = ucfirst($row['store_name']) . (strlen($row['address']) > 0 ? ' - ' : '') . ucfirst($row['address']);
            }
        }
        $query->free_result();
        return $data;
