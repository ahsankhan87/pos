...
class C_delivery_note extends MY_Controller
{
    ...
    public function index($saleType = '', $customer_id = '', $estimate_no = '')
    {
        ...
        $data['title'] = 'Create delivery note';
        $data['main'] = 'Create delivery note';
        ...
        $this->load->view('pos/delivery_note/v_salesProduct', $data);
    ...
    public function all()
    {
        ...
        $data['main'] = "Delivery Note";

        //$data['main_small'] = $fiscal_dates;

        $data['sales'] = $this->M_delivery_note->get_sales(false, $start_date, $to_date);

        $this->load->view('templates/header', $data);
        $this->load->view('pos/delivery_note/v_all', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function editSales($invoice_no)
    {
        ...
        $this->load->view('templates/header', $data);
        $this->load->view('pos/delivery_note/v_editSalesProduct', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function saleProducts()
    {
        ...
        if (count((array)$data_posted) > 0) {
            ...
            @$prev_invoice_no = $this->M_delivery_note->getMAXSaleInvoiceNo();
            ...
            $delivery_date = ($data_posted->delivery_date == '' ? '' :date('Y-m-d', strtotime($data_posted->delivery_date)));
            ...
            //if(count($sale_supp_posting_type_code) !== 0)
            {
                ...
                $this->db->insert('pos_delivery_note', $data);
                ...
                foreach ($data_posted->items as $posted_values) :
                    ...
                    $this->db->insert('pos_delivery_note_items', $data);
                ...
                elseif ($data_posted->saleType == 'credit' && $data_posted->register_mode == 'return') {
                    ...
                    if ($supplier_id) {
                        ...
                        $creditSales = $this->M_delivery_note->get_creditSales($customer_id);
                        foreach ($creditSales as $values) {
                            ...
                            $this->db->update('pos_delivery_note', $data, array('invoice_no' => $values['invoice_no']));
    ...
    {
        $data = array('langs' => $this->session->userdata('lang'));
        $data['sales_items'] = $this->M_delivery_note->get_sales_items($new_invoice_no);
        ...
        $this->load->view('templates/header', $data);
        // $this->load->view('pos/delivery_note/v_receipt_small',$data);
        $this->load->view('pos/delivery_note/v_receipt', $data);
        $this->load->view('templates/footer');
    }
    ...
    function get_sales_JSON()
    {
        ...
        print_r(json_encode($this->M_delivery_note->get_selected_sales($start_date, $to_date)));
    }
    ...
    public function delete($invoice_no, $redirect = true)
    {
        ...
        $sales_items = $this->M_delivery_note->get_sales_items($invoice_no);
        ...
        $this->M_delivery_note->delete($invoice_no);
        $this->db->trans_complete();
    ...
    {
        $data = $this->M_delivery_note->get_sales_items_only($invoice_no);
    ...
    {
        $data = $this->M_delivery_note->get_sales_by_invoice($invoice_no);
    ...
    {
        $sales_items = $this->M_delivery_note->get_sales_items($new_invoice_no);
        //$sales_items = $data['sales_items'];
    ...
    {
        $data = array('langs' => $this->session->userdata('lang'));
        $data['sales_items'] = $this->M_delivery_note->get_sales_by_invoice($new_invoice_no);
        $data['invoice_no'] = $new_invoice_no;

        $data['title'] =  'Sales';
        $data['main'] = ''; //($sales_items[0]['register_mode'] == 'sale' ? 'Sales' : 'Return').' Invoice #'.$new_invoice_no;

        $this->load->view('pos/delivery_note/receipt_ubl_xml', $data);
    }
