...
class M_items extends CI_Model
{
    ...
    function getItemDropDown($item_type = null)
    {
        ...
        $query = $this->db->get_where('pos_items_detail AS A', array(
            'B.deleted' => 0, 'A.deleted' => 0,
    ...
    public function get_allItemsforJSON($search = "", $limit = 1000000, $offset = 0)
    {
        if ($limit) {
            $this->db->limit($limit);
            $this->db->offset($offset);
        ...
        if ($search != "") {
            ...
            $this->db->or_where('A.item_id', $search);
            $this->db->or_where('A.barcode', $search);
        }
