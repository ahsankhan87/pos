...
class C_recurring_invoices extends MY_Controller
{
    ...
    public function index($saleType = '', $customer_id = '', $estimate_no = '')
    {
        ...
        $data['title'] = 'Schedule Recurring Invoice';
        $data['main'] = 'Schedule Recurring Invoice';
        ...
        $this->load->view('pos/recurring_invoices/v_salesProduct', $data);
    ...
    public function all()
    {
        ...
        $data['title'] = "Recurring Invoices";
        $data['main'] = "Recurring Invoices";

        //$data['main_small'] = $fiscal_dates;

        $data['sales'] = $this->M_recurring_invoices->get_sales(false, $start_date, $to_date);

        $this->load->view('templates/header', $data);
        $this->load->view('pos/recurring_invoices/v_all', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function editSales($invoice_no)
    {
        ...
        $this->load->view('templates/header', $data);
        $this->load->view('pos/recurring_invoices/v_editSalesProduct', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function saleProducts()
    {
        ...
        if (count((array)$data_posted) > 0) {
            ...
            @$prev_invoice_no = $this->M_recurring_invoices->getMAXSaleInvoiceNo();
            ...
            $period_start_date = ($data_posted->period_start_date == '' ? '' :date('Y-m-d', strtotime($data_posted->period_start_date)));
            $last_send_date = ($data_posted->last_send_date == '' ? '' :date('Y-m-d', strtotime($data_posted->last_send_date)));
            $send_every_day = $data_posted->send_every_day;
            $send_every_month = $data_posted->send_every_month;
            $next_shipment = $data_posted->next_shipment;
            ...
            //if(count($sale_supp_posting_type_code) !== 0)
            {
                ...
                $data = array(
                    ...
                    'send_every_day'=>$send_every_day,
                    'send_every_month'=>$send_every_month,
                    'next_shipment'=>$next_shipment,
                    
                );

                $this->db->insert('pos_recurring_invoices', $data);
                ...
                foreach ($data_posted->items as $posted_values) :
                    ...
                    $this->db->insert('pos_recurring_invoices_items', $data);
                ...
                elseif ($data_posted->saleType == 'credit' && $data_posted->register_mode == 'return') {
                    ...
                    if ($supplier_id) {
                        ...
                        $creditSales = $this->M_recurring_invoices->get_creditSales($customer_id);
                        foreach ($creditSales as $values) {
                            ...
                            $this->db->update('pos_recurring_invoices', $data, array('invoice_no' => $values['invoice_no']));
    ...
    {
        $data = array('langs' => $this->session->userdata('lang'));
        $data['sales_items'] = $this->M_recurring_invoices->get_sales_items($new_invoice_no);
        ...
        $this->load->view('templates/header', $data);
        // $this->load->view('pos/recurring_invoices/v_receipt_small',$data);
        $this->load->view('pos/recurring_invoices/v_receipt', $data);
        $this->load->view('templates/footer');
    }
    ...
    function get_sales_JSON()
    {
        ...
        print_r(json_encode($this->M_recurring_invoices->get_selected_sales($start_date, $to_date)));
    }
    ...
    public function delete($invoice_no, $redirect = true)
    {
        ...
        $sales_items = $this->M_recurring_invoices->get_sales_items($invoice_no);
        ...
        $this->M_recurring_invoices->delete($invoice_no);
        $this->db->trans_complete();
    ...
    {
        $data = $this->M_recurring_invoices->get_sales_items_only($invoice_no);
    ...
    {
        $data = $this->M_recurring_invoices->get_sales_by_invoice($invoice_no);
    ...
    {
        $sales_items = $this->M_recurring_invoices->get_sales_items($new_invoice_no);
        //$sales_items = $data['sales_items'];
    ...
    {
        $data = array('langs' => $this->session->userdata('lang'));
        $data['sales_items'] = $this->M_recurring_invoices->get_sales_by_invoice($new_invoice_no);
        $data['invoice_no'] = $new_invoice_no;

        $data['title'] =  'Sales';
        $data['main'] = ''; //($sales_items[0]['register_mode'] == 'sale' ? 'Sales' : 'Return').' Invoice #'.$new_invoice_no;

        $this->load->view('pos/recurring_invoices/receipt_ubl_xml', $data);
    }
